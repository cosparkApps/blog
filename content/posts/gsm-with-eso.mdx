---
title: 用 Google Secret Manager 搭配 External Secret Operator 管理 GKE 機敏資料
description: 本文介紹如何在 GKE 環境使用 Google Secret Manager (GSM) 集中管理機敏資料，並透過 Workload Identity (WI) 與 External Secret Operator (ESO) 實現 K8s Secret 自動同步，附上 Terraform 與 YAML 實作範例。
date: 2025-5-27 08:00:00 +0800
tags: [Security, GCP, Kubernetes, Operator, GSM, ESO, WorkloadIdentity, ExternalSecrets, DevOps]
---

## TL;DR
- 使用 [Google Secret Manager (GSM)](https://cloud.google.com/security/products/secret-manager) 集中管理機敏資料  
- 透過 [Workload Identity (WI)](https://cloud.google.com/iam/docs/workload-identity-federation) 授予 [External Secret Operator (ESO)](https://external-secrets.io/latest/provider/google-secrets-manager/) 存取 GSM 權限  
...
### 部署 External Secret Operator (ESO)

在 Helm values 中指定 KSA 對應的 GSA：<br/>
...
```
👉 接著建立 ClusterSecretStore 讓 ESO 知道 `secret` 來源:
