---
title: 用 CRD 解決 GKE 資源不同步的問題
description: 在 GKE 環境中，CI/CD pipeline 使用 kubectl set image 容易造成資源與 Helm Release 不同步。本文比較純腳本、--reuse-value、CRD 三種解法，並提供完整 CRD + CR YAML 範例，協助解決資源漂移問題。
